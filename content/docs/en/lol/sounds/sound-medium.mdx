...
When generating SoundBanks, Wwise can bundle structure and media into a single bank. Splitting is optional, but if you merge events and media together, other modders can't inspect or replace your sounds in Quartz. To split, generate:

1. An `_events.bnk` holding the structure and event definitions.
2. An `_audio.bnk` (or `.wpk`) holding media only.

If you don't split, put everything in `_events.bnk`.
